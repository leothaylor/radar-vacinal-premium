...
- `js/engine.js` passa a ser um loader mínimo; `js/engine-release-patch.js` aplica o hotfix sobre o motor preservado.
...
- `engine-core.js` e `engine-release-patch.js` adicionados ao precache do PWA.
...
O wrapper é deliberadamente reversível. Em futura refatoração, incorporar a semântica inclusiva de `maxMonths` diretamente ao motor e mover o Measurement ID para a configuração principal; somente então remover `engine-core.js`/loader/patch.
